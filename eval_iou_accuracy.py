...
file_name = './test_results/ring_vdm_asm/output_filelist.txt'

pred_data_label_filenames += [line.rstrip() for line in open(file_name)]
LOG_FOUT = open(os.path.join('results_ring_vdm_asm.txt'), 'w')
gt_label_filenames = [f.rstrip('_pred\.txt') + '_gt.txt' for f in pred_data_label_filenames]
# print('gt_label_filenames',gt_label_filenames)
...
for i in range(num_room):
    ...
    pred_sem = np.zeros((pred_ins.shape))
    gt_label = np.loadtxt(gt_label_filenames[i])
    # gt_ins = gt_label[:,-1].reshape(-1).astype(np.int)
    gt_ins = gt_label.reshape(-1).astype(np.int)

    gt_sem = np.zeros((gt_ins.shape))
